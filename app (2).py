...
def load_xception_model(model_path):
    img_shape = (150, 150, 3)  # Reduced size for memory management
...
def load_custom_cnn_model():
    ...
    model.compile(optimizer=Adamax(learning_rate=0.001),
                  loss='categorical_crossentropy',
                  metrics=['accuracy', Precision(), Recall()])
    model.load_weights("/content/cnn_model.h5")
    return model
...
if uploaded_file is not None:
    ...
    # Display uploaded image and saliency map
    ...
    # Probability breakdown with a custom bar chart
    ...
    # Clear model from memory after predictions
    del model
    gc.collect()
